...
class UpdateLegacyProfileRequest extends FormRequest
{
    public function authorize(): bool
    {
        return $this->user()?->can('manage', $this->route('legacyProfile')) ?? false;
    }
    ...
    public function rules(): array
    {
        ...
        $legacyProfile = $this->route('legacyProfile');

        return [
            'legacy_external_key' => ['required', 'string', 'max:191', 'regex:/\A[A-Za-z0-9._:-]+\z/', Rule::unique(LegacyProfile::class)->ignore($legacyProfile)],
            'nickname' => ['required', 'string', 'max:120'],
            'slug' => ['required', 'string', 'max:150', 'alpha_dash', Rule::unique(LegacyProfile::class)->ignore($legacyProfile)],
            'legacy_joined_at' => ['nullable', 'date'],
            'legacy_rank' => ['nullable', 'string', 'max:120'],
            'legacy_message_count' => ['required', 'integer', 'min:0', 'max:4294967295'],
            ...
            'legacy_avatar_remove' => ['nullable', 'boolean'],
