...
class UpdateForumRequest extends FormRequest
{
    ...
    public function rules(): array
    {
        ...
        return ['forum_category_id' => ['required', 'exists:forum_categories,id'], 'name' => ['required', 'string', 'max:120'], 'slug' => ['required', 'max:150', 'alpha_dash', Rule::unique(Forum::class)->ignore($forum)], 'description' => ['nullable', 'string', 'max:3000'], 'icon' => ['nullable', 'string', 'max:80'], 'sort_order' => ['required', 'integer', 'min:0'], 'is_active' => ['sometimes', 'boolean'], 'is_locked' => ['required', 'boolean'], 'minimum_role' => ['nullable', Rule::in(['user', 'moderator', 'admin'])]];
    }
}
