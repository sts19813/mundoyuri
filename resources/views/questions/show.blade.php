...
<main class="portal-profile-page forum-page question-page"><div class="container-xl px-4 position-relative community-conversation-shell">
    ...
    <x-forum.heading :eyebrow="$thread->isResolved() ? 'Pregunta resuelta' : 'Pregunta abierta'" :title="$thread->title">
        <p>{{ number_format($thread->replies_count) }} {{ $thread->replies_count === 1 ? 'respuesta' : 'respuestas' }} · {{ number_format($thread->views_count) }} vistas</p>
        <x-slot:actions>
            @auth
                @can('reply', $thread)<a href="#responder" class="profile-btn profile-btn-primary">Responder</a>@endcan
            @else
                <a href="{{ route('login') }}" class="profile-btn profile-btn-primary">Participar</a>
            @endauth
            <a href="{{ route('questions.index') }}" class="profile-btn profile-btn-soft">Ver preguntas</a>
            @auth
                <details class="forum-action-menu">
                    <summary aria-label="Acciones de la pregunta">•••</summary>
                    <div class="forum-action-menu-panel"><x-community.report-form :reportable="$thread" /></div>
                </details>
            @endauth
        </x-slot:actions>
    </x-forum.heading>
    ...
    <section class="forum-posts">@foreach($postTree as $post)<x-forum.thread-post :post="$post" :question="$thread" :accepted-answer-post-id="$thread->accepted_answer_post_id" />@endforeach</section>
    @if($focusedPostId)<p class="forum-load-all"><a href="{{ route('questions.show', $thread) }}">Ver la conversación desde el inicio</a></p>@elseif($hasMoreReplies)<p class="forum-load-all"><a href="{{ route('questions.show', $thread) }}?all=1">Ver las {{ number_format($thread->replies_count) }} respuestas</a></p>@elseif($showAllReplies && $thread->replies_count > 100)<p class="forum-load-all"><a href="{{ route('questions.show', $thread) }}">Ver las primeras 100 respuestas</a></p>@endif
    @auth
