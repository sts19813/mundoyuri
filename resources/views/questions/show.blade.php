...
<main class="portal-profile-page forum-page question-page"><div class="container-xl px-4 position-relative">
    ...
    <header class="forum-thread-header question-thread-header"><div><span class="profile-eyebrow">{{ $thread->isResolved() ? 'Pregunta resuelta' : 'Pregunta abierta' }}</span><h1>{{ $thread->title }}</h1><div class="question-tags">@foreach($thread->questionTags as $questionTag)<a href="{{ route('questions.index', ['tag' => $questionTag->slug]) }}">{{ $questionTag->name }}</a>@endforeach</div><p>{{ number_format($thread->views_count) }} vistas · {{ number_format($thread->replies_count) }} respuestas</p></div><a href="{{ route('questions.index') }}" class="profile-btn profile-btn-soft">Ver preguntas</a></header>
