            ...
            <div class="community-member-grid">
                @forelse($legacyProfiles as $legacyProfile)
                    <article class="community-member-card legacy-profile-card">
                        ...
                        <div class="community-member-copy"><div class="community-member-title"><div><h3><a href="{{ route('legacy-profiles.show', $legacyProfile) }}">{{ $legacyProfile->nickname }}</a></h3><small>Perfil histórico</small></div>@if($legacyProfile->legacy_rank)<span class="community-rank">{{ $legacyProfile->legacy_rank }}</span>@endif</div><div class="community-member-badges">@foreach($legacyProfile->badges as $badge)<x-community.badge :badge="$badge" />@endforeach</div><dl class="community-member-stats"><div><dt>Registro histórico</dt><dd>{{ $legacyProfile->legacy_joined_at?->translatedFormat('M Y') ?: 'Sin fecha' }}</dd></div><div><dt>Mensajes archivados</dt><dd>{{ number_format($legacyProfile->legacy_message_count) }}</dd></div></dl></div>
                    </article>
                @empty
