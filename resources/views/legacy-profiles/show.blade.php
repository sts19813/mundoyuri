...
<body>
    ...
    <main class="portal-profile-page">
        ...
        <div class="container-xl px-4 position-relative">
            ...
            <div class="profile-grid mt-4"><section class="profile-panel profile-panel-main"><div class="profile-panel-heading"><div><span class="profile-panel-kicker">Archivo de Mundo Yuri</span><h2>Datos recuperados</h2></div></div><dl class="profile-details-list legacy-profile-details"><div><dt>Registro histórico</dt><dd>{{ $legacyProfile->legacy_joined_at?->translatedFormat('d M Y') ?: 'Sin fecha disponible' }}</dd></div><div><dt>Mensajes archivados</dt><dd>{{ number_format($legacyProfile->legacy_message_count) }}</dd></div>@if($legacyProfile->legacy_rank)<div><dt>Rango antiguo</dt><dd>{{ $legacyProfile->legacy_rank }}</dd></div>@endif
